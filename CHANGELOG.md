...
- A control scheme's device family is derived from the devices it requires instead of being authored: a scheme with a pointer is pointer based, one with a gamepad is gamepad based, one with both is both, and one with neither is undefined. Layouts match by inheritance, so a mouse, pen or touchscreen all count as pointers and a DualShockGamepad counts as a gamepad. The Control Scheme Device Families section is gone, and `ControlScheme.IsMouseBased()` becomes `IsPointerBased()`
