...
- A control scheme's device family is derived from the devices it requires instead of being authored: a scheme with a mouse is mouse based, one with a gamepad is gamepad based, neither is undefined. Layouts match by inheritance, so a DualShockGamepad counts as a gamepad. The Control Scheme Device Families section is gone
