        ...
        internal static void AddContent(string markerName, InputActionMap map, List<string> lines)
        {
            ...
            switch (markerName)
            {
                ...
                case "ConstructorSignature":
                    lines.Add($"        internal {className()}(int playerID, {nameof(InputActionAsset)} asset, Dictionary<Guid, {nameof(ActionWrapper)}> table)");
                    break;
                ...
                case "ActionWrapperAssignments":
                    foreach (string action in getActionNames())
                        lines.Add($"            {action.AsProperty()} = new (playerID, {actionMapProperty()}.FindAction(\"{action}\", throwIfNotFound: true), table);");
                    break;
